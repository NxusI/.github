# NxsusI

<p align="center">
  <img src="/NexusOne/IdViz/NomeEquipeFull.svg" alt="Logomarca da Equipe Nexus One" width="200"/>
</p>

## 📝 Descrição da Equipe e Projeto

Nós somos a equipe **Nexus One**, um grupo de estudantes do curso de **Sistemas e Mídias Digitais** do departamento **UFC-Virtual** da **Unversidade Federal do Ceará**, dedicado a aplicar nossos conhecimentos em desenvolvimento de software e outras áreas para gerar uma solução inovadora para a disciplina **Projeto Integrado I**

Este repositório hospeda o projeto **Sistema Integrado de Gestão da Célula de Manutenção (SIG-Manutenção)**, desenvolvido em parceria com o **Instituto Robótica Sustentável**.

**O Desafio:** A célula de Manutenção da ONG, que é crucial para gerar receita e fornecer insumos para a educação em robótica, opera sem um sistema unificado. Essa lacuna resulta em processos informais, perda de documentação, ineficiência no fluxo de atendimento ao cliente (externo) e incapacidade de realizar análises financeiras e de desempenho.

**A Solução (SIG-Manutenção):** Nosso sistema visa transformar essa realidade, digitalizando o fluxo de Ordens de Serviço (OS) e as operações financeiras da célula. O SIG-Manutenção permitirá o **registro unificado**, **gerenciamento de prazos e garantias**, e **análise do desempenho financeiro**, garantindo maior profissionalismo, transparência e eficiência para as atividades da ONG.

---

## 🏛️ Identificação Institucional e Cliente

### Instituto Robótica Sustentável

| Instituição | Contato Principal | Foco |
| :--- | :--- | :--- |
| **Instituto Robótica Sustentável (ONG)** | André Cardoso (Presidente) | Educação, Sustentabilidade Tecnológica e Robótica. |

### Equipe

* **Universidade:** Universidade Federal do Ceará
* **Unidade Acadêmica:** UFC Virtual
* **Curso:** Sistemas e Mídias Digitais
* **Disciplina:** Projeto Integrado I

---

## 👥 Membros da Equipe - Nexus One

| Função no Projeto | Nome Completo | GitHub / Contato |
| :--- | :--- | :--- |
| Membro da Equipe | **Brunna Raiany Moura Dos Santos** | [Link do GitHub ou LinkedIn] |
| Membro da Equipe | **Fernando De Oliveira Barros** | [Link do GitHub ou LinkedIn] |
| Membro da Equipe | **Gabriel Vitor Sales Nascimento** | [Link do GitHub ou LinkedIn] |
| Membro da Equipe | **Maria Leydiane De Melo Conceição** | [Link do GitHub ou LinkedIn] |
| Membro da Equipe | **Samuel Bruno Silva Secunde** | [Link do GitHub ou LinkedIn] |
| Membro da Equipe | **Túlio de Oliveira Vasconcelos** | [Link do GitHub ou LinkedIn] |
| **Professor(a) Orientador(a) (Opcional)** | [Nome do Professor(a)] | [E-mail ou Contato Institucional] |